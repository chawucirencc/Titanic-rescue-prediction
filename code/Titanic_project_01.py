...
def deal_missing_data(all_Data):
    ...
    # 创建新属性Title
    def get_title(name):
        tit_search = re.search(', [A-Za-z]+.', name)
        return tit_search.group(0).strip(', .')
    title = all_Data['Name'].apply(get_title)
    tit_dic = {'Mr': 1, 'Miss': 2, 'Mrs': 3, 'Master': 4, 'Dr': 5, 'Rev': 6, 'Mlle': 7,'Col': 8, 'Major': 9,
               'Lady': 10, 'Ms': 11, 'Mme': 12, 'Don': 13, 'Jonkheer': 14, 'the': 15, 'Capt': 16, 'Sir': 17, 'Dona':18}
    for k, v in tit_dic.items():
        title[title==k] = v
    all_Data['Title'] = title

    # def get_new_parch(parch):
    #     if parch == 0:
    #         return 1
    #     elif 0 < parch:
    #         return 2
    # New_Parch = all_Data['Parch'].apply(get_new_parch)
    # all_Data['New_PArch'] = New_Parch
    # 创建新属性New_Family
    all_Data['Family'] = all_Data['Parch'] + all_Data['SibSp']
    def get_new_family(family):
        if family == 0:
            return 1
        elif 0 < family:
            return 2
    New_Family = all_Data['Family'].apply(get_new_family)
    all_Data['New_Family'] = New_Family
    # print(pd.value_counts(all_Data['Fare']))
...
def deal_data(complete_data):
    ...
    data = pd.concat([complete_data, dummies_cabin, dummies_pclass, dummies_sex, dummies_embarked], axis=1)
    drop = ['Cabin', 'Sex', 'Embarked', 'Pclass', 'Name', 'Ticket','PassengerId', 'Embarked_Q', 'Family']
    data.drop(drop, axis=1, inplace=True)
    # 画出特征评分图
    # feature = data.columns
    # select = SelectKBest(f_classif, k=5)
    # select.fit(data[feature], data['Survived'])
    # score = -np.log10(select.pvalues_)
    # plt.rcParams['font.sans-serif'] = ['SimHei']
    # plt.bar(range(len(feature)), score)
    # plt.xlabel('特征名称'); plt.ylabel('评分'); plt.title('各个特征评分')
...
def create_model(X, y):
    ...
    result = cross_val_score(log_model, X_train, y_train, scoring='accuracy')
